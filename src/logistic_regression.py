...
def model_tuning_logistic(training_path, testing_path, text_col, sentiment_col, logistic_n_grams, logistic_Cs, logistic_penalty, output_path):
    # get data
    ...
    for n_gram in logistic_n_grams:
        ...
        for c in logistic_Cs:
            for p in logistic_penalty:
                ...
                print(model_name)
                model = LogisticRegression(random_state=1, solver='liblinear', penalty=p, C=c)
                clf = model.fit(X_train_tfidf, y_train)
                accuracy, precision, recall, f1 = model_evaluation(clf, X_test_tfidf, y_test)
